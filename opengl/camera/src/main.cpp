...
void createCube(std::vector<TriangleVertex>& vertices, std::vector<unsigned int>& indices){
...
int main(void) {
	...
	//carrega os shaders
	if (!readFile("resources/shaders/vertex_shader_camera.gl", vertexShaderSource) || !readFile("resources/shaders/fragment_shader_multi_texture.gl", fragmentShaderSource)) {
		glfwTerminate();
		return -1;
	}
	...
	for (unsigned int i = 0; i < 100; i++) {
		...
		offsetLocation = glGetUniformLocation(shaderProgram, ("offsets[" + ss.str() + "]").c_str());
	...
	while (!glfwWindowShouldClose(window)) {
		...
		glBindVertexArray(vaoId);
		glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, indicesId);
		glDrawElementsInstanced(GL_TRIANGLES, indices.size(), GL_UNSIGNED_INT, nullptr, 100);
