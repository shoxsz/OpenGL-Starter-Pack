...
int main(void) {
	...
	//carrega os shaders
	if (!readFile("resources/shaders/vertex_shader_projections.glsl", vertexShaderSource) || !readFile("resources/shaders/fragment_shader_multi_texture.glsl", fragmentShaderSource)) {
		glfwTerminate();
		return -1;
	}
